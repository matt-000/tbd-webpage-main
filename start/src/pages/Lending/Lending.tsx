...
const App = () => {
  ...
  const stringVal = (num: bigint | null) => {
    ...
    const value =
      quotient.toString() + "." + remainder.toString().padStart(18, "0");
  ...
  const [stringConversionRate, setStringConversionRate] =
  ...
  const updateBalance = async () => {
    ...
    let fettiContract = new ethers.Contract(
      context!.fetti_address,
      iface.fragments,
      fettiSigner
    );
  ...
  return (
    <div className="app">
      ...
      <Minty
        ...
        fetti_address={context!.fetti_address}
        provider={provider}
        signer={signer}
        contract={contract}
        tokenName={tokenName}
        tokenNameSC={tokenNameSC}
        balance={balance}
        balanceSC={balanceSC}
        updateBalance={updateBalance}
        stringConversionRate={stringConversionRate}
      />
    </div>
  );
};
